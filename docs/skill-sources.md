...
org vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. org
...
#   - consumer-identifying prose: `guildmaster` → `org` (NOT
...
If a re-sync would lose a org adaptation, lift the change
...
#     → `which org vendors from guildmaster`
