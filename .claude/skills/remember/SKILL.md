...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope org
...
scope:** the default personal scope is **private** (`--scope org
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `org`). Private records are served only to a recall in the same
